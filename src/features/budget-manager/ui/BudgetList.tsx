...
export function BudgetList() {
  ...
  useEffect(() => {
    setBudgets(getBudgetsByMonth(month));
  ...
  const handleSetBudget = (categoryId: string) => {
    ...
    upsertBudget(budget);
    setBudgets((prev) => [...prev.filter((b) => b.categoryId !== categoryId), budget]);
  };
  ...
  return (
    <ScrollView className="p-4">
      ...
      {categories.map((cat) => {
        ...
        const amountStr = editingAmounts[cat.id] ?? budget?.amount.toString() ?? "";
